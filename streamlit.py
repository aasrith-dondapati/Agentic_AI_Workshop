...
st.title("🏏 CRICKET AGENTIC AI")
st.subheader("Get live cricket match scores, player statistics, and news updates.")

col1, col2 = st.columns(2)
with col1:
    match_query = st.text_input("Enter match query")
with col2:
    player_query = st.text_input("Enter player query")

if st.button("Get Live Match Updates"):
    query = 'Get latest score'
    if match_query:
        query += f" for {match_query}"
    if player_query:
        query += f" for {player_query}"
    query += " , and latest news"
